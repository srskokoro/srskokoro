...
internal object AppSwing {

	@Suppress("NOTHING_TO_INLINE")
	inline fun ensureLaf() {
		val ex = `-thrown`
		if (ex != null) throw wrap(ex)
	}

	@Suppress("NOTHING_TO_INLINE")
	inline fun ensureLaf(component: Component) {
		ensureLaf()
	...
	@JvmField internal var `-thrown`: Throwable? = null
	...
	init {
		...
		// Must manually invoke the listener 'right after' registering it (and
		// not before it), as the listener will only be called after an OS theme
		// change is detected, i.e., it won't be called until then.
		listener.accept(detector.isDark) // NOTE: May set `-thrown`
		...
		// won't be called during registration, but rather, only after another
		// OS theme change.
	}
